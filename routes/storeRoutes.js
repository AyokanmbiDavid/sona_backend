...
router.get('/latest',authMiddleware,(req,res) => {
    const getNew = store.filter(item => item.spec == 'new');

   return res.status(201).json(getNew)
})

router.post('/search',authMiddleware,(req,res) => {
    let {searchval} = req.body;

    let filtersearch = store.filter(item => item.title.toLocaleLowerCase().includes(searchval.toLocaleLowerCase()));

    return res.status(201).json(filtersearch)
})

router.post('/category',authMiddleware,(req,res) => {
    const {item} = req.body
    const getCategory = store.filter(storeitem => storeitem.category == item);
